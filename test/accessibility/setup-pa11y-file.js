...
try {
  ...
  .then(function(courtReportInserts) {
    ...
    return aggregatedDataHelper.addWorkloadCapacitiesForOffenderManager().then(function(workloadInserts) {
      ...
      const offenderManagerId = workloadInserts.find((item) => item.table === 'workload_owner').id
      const teamId = workloadInserts.find((item) => item.table === 'team').id
      const lduId = workloadInserts.find((item) => item.table === 'ldu').id
      const regionId = workloadInserts.find((item) => item.table === 'region').id
      const courtReporterId = courtReportInserts.find((item) => item.table === 'workload_owner').id
